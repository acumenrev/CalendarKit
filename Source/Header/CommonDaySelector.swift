...
class CommonDaySelector: UIView, ReusableView {
    ...
    var dateLabelHeight: CGFloat = 44
    ...
    func initializeViews() {
        for _ in 1...daysInWeek {
            ...
            let recognizer = UITapGestureRecognizer(target: self,
                                                    action: #selector(CommonDaySelector.dateLabelDidTap(_:)))
    ...
    override func layoutSubviews() {
        ...
        dateLabelWidth = frame.size.width/(dateLabelsCount) - 2
        ...
        for (i, label) in dateLabels.enumerated() {
            ...
            label.frame = frame
            label.initLine(width: Int(frame.width), height: Int(frame.height))
        }
